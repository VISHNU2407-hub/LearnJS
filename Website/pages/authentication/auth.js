/* ============================================================
   LearnJS — auth.js (pages/authentication)
   Page-level authentication logic for login.html and signup.html:
   form validation, password strength, forgot-password, Google
   sign-in and post-auth redirect to the dashboard.
   Loaded as a module so it can import firebase.js.
   ============================================================ */

import { auth, googleProvider } from "../../js/firebase/firebase.js";
import {
...
/** Default destination after auth: the dashboard. A `?next=` param wins. */
function nextPath() {
  try {
    const raw = new URLSearchParams(window.location.search).get("next");
    if (!raw) return "../dashboard/";
    // SECURITY: only allow same-origin relative navigation. This blocks open
    // redirects (login.html?next=https://evil.com) and script execution via
    // crafted schemes (login.html?next=javascript:alert(1)).
...
function init() {
  ...
  // If the user is already signed in, skip the auth form entirely and go
  // straight to the dashboard (or the ?next= destination) — no extra click.

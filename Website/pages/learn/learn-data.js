...
window.LEARNJS_WORKSHOPS = {
  ...
  "blog-website": {
    ...
    time: "60 min",
    category: "Core JS",
    tags: ["DOM", "Events", "LocalStorage", "SPA", "Search"],
    intro: "Build a full-featured blog as a single-page application using vanilla JavaScript. You'll store articles as data, render cards dynamically, filter by category, search across posts, bookmark and like articles, leave comments, and switch between a home page, articles listing, single article view, and bookmarks — all without page reloads.",
    previewNote: "You'll build a blog that keeps its articles in a JavaScript array, renders each one as a card with author avatars, lets visitors filter by category and search by keyword, bookmark and like their favorite posts, leave comments, and switch between multiple views — all powered by vanilla JavaScript and localStorage for persistence.",
    cover: "../../assets/project-covers/blog-website.png",
    previewUrl: "../../../JS%20PROJECTS/blog%20website/index.html",
    githubUrl: "https://github.com/VISHNU2407-hub/LearnJS",
    communityUrl: "../community/",
    tips: [
      "Keep the article data in one array so rendering, filtering and searching all share the same source of truth.",
      "Render cards with a loop (forEach or map) so adding a new article automatically appears on the page.",
      "Use event delegation (one listener on the parent) instead of attaching a click listener to every card.",
      "Store bookmarks, likes and comments in localStorage so they survive a page refresh.",
      "Use template literals (backtick strings) to build HTML — they support multi-line strings and variable interpolation.",
      "console.log() the array you're rendering when a card looks wrong — the data is usually the culprit."
    ],
    concepts: [
      "Single-page application routing with show/hide panels",
      "Storing content as JavaScript data (arrays of objects)",
      "Rendering lists to the DOM with template literals",
      "Filtering and searching data with array methods",
      "localStorage for bookmarks, likes and comments",
      "Event delegation on dynamic elements",
      "Reading progress indicator with scroll events"
    ],
    challenge: "Extra challenge: add a dark mode toggle that persists in localStorage, or implement article creation with a form that saves new posts to the article array.",
    steps: [
      {
        title: "Project Setup",
        tagline: "Understand the starter files.",
